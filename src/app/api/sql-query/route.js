...
export async function POST(request) {
  try {
    ...
    const { connectionString, connectionId, organizationId, databaseName, sqlQuery } = body || {};

    // Validate required fields
    if ((!connectionString && (!connectionId || !organizationId)) || !databaseName || !sqlQuery) {
      return NextResponse.json(
        {
          success: false,
          error: 'Either connectionString or (connectionId and organizationId), database name, and SQL query are required'
        },
        { status: 400 }
      );
    }
