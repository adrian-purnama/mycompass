          ...
          {results && (
            <div className="flex-1 flex flex-col overflow-hidden min-h-0">
              ...
              {viewFormat === "table" && (
                ...
                >
                  <div
                    className="flex-1 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-md min-h-0"
                    style={{ minWidth: 0, width: "100%", overflow: "hidden", display: "flex", flexDirection: "column" }}
                  >
                    <div
                      style={{
                        flex: 1,
                      ...
                      >
                        <thead className="bg-zinc-100 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
                          <tr>
                            {getTableData.headers.map((header, idx) => (
                              <th
                                key={idx}
                                className="px-2 py-1.5 text-left font-semibold text-black dark:text-zinc-50 sticky top-0 bg-zinc-100 dark:bg-zinc-900 z-10 whitespace-nowrap"
                              >
                                {header}
                              </th>
                            ))}
                        ...
                        <tbody>
                          {getTableData.rows.map((row, rowIdx) => {
                            const actualRowIdx =
                              (currentPage - 1) * rowsPerPage + rowIdx;
                            ...
                            return (
                              ...
                              >
                                {row.map((cell, cellIdx) => {
                                  const fullValue =
                                    results[actualRowIdx]?.[
                                      getTableData.headers[cellIdx]
                                    ];
