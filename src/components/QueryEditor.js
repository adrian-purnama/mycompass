        ...
        <div className="flex-1 flex flex-col min-h-0 min-w-0">
          <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex-shrink-0">
            ...
            <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
              Supported fields: filter, sort, limit, skip, type (find/aggregate), pipeline (for
              aggregate)
            </p>
          </div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                  <div className="text-sm text-zinc-500 dark:text-zinc-400">
                    Showing {((currentPage - 1) * rowsPerPage) + 1} to {Math.min(currentPage * rowsPerPage, results.length)} of {results.length} results
                  </div>
                  <div className="flex items-center gap-2">
                    <button
